...
class InterfaceServiceProvider extends ServiceProvider
{
    ...
    {
        return DB::table('commerciauxes')->where('codeCom', $code)->first();
    }
    ...
    public static function infosup($id)
    {
        if ($id == null || $id == 0) {
            ...
            $hs = DB::table('hierarchies')->where('codeH', $id)->first();
            
            if (!$hs) {
                return "n'exite pas";
            }
            if(isset($hs->managerH) && $hs->managerH == 0)
                return "inconnue";
            if(isset(self::infocom($hs->managerH)->codeCom)) 
				return self::infocom($hs->managerH)->nomCom.' '.self::infocom($hs->managerH)->prenomCom;
			else;
				return "";
        }
    }
